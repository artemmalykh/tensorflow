...
const bool PlainClient::IsConnected() { return sock != -1; }

const int PlainClient::GetSocketDescriptor() { return sock; }

const tensorflow::Status PlainClient::ReadData(char* buf, int length) {
  int recieved = 0;
...
const tensorflow::Status PlainClient::Ignore(int toIgnore) {
  int recieved = 0;

  char c = 0;

  while (recieved < toIgnore) {
...
const tensorflow::Status PlainClient::WriteData(const char* buf, int length) {
  int sent = 0;
